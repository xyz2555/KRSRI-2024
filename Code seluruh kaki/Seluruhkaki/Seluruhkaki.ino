...
//  delay(2000);
  ...
  gerak(0,0,0,"kananbelakang", hasil_kananb);
  Serial.println(hasil_kananb[0]);
  Serial.println(hasil_kananb[1]);
  Serial.println(hasil_kananb[2]);
  delay(5000);
}

void gerak(double x, double y, double z, String kaki, double hasil[3] ) {
...
void gerakservo(int belakangka1, int belakangka2, int belakangka3, int belakangki1, int belakangki2, int belakangki3, int depanki1, int depanki2, int depanki3, int depanka1, int depanka2, int depanka3, double hasilkab[3], double hasilkib[3], double hasilkid[3], double hasilkad[3]){
  ...
  double konversi1 = map(hasilkab[0], 300, 0, 0, 1024);
  double konversi2 = map(hasilkab[1], 300, 0, 0, 1024);
  double konversi3 = map(hasilkab[2], 300, 0, 0, 1024);

  double konversi4 = map(hasilkib[0], 300, 0, 0, 1024);
  double konversi5 = map(hasilkib[1], 300, 0, 0, 1024);
  double konversi6 = map(hasilkib[2], 300, 0, 0, 1024);

  double konversi7 = map(hasilkid[0], 300, 0, 0, 1024);
  double konversi8 = map(hasilkid[1], 300, 0, 0, 1024);
  double konversi9 = map(hasilkid[2], 300, 0, 0, 1024);

  double konversi10 = map(hasilkad[0], 300, 0, 0, 1024);
  double konversi11 = map(hasilkad[1], 300, 0, 0, 1024);
